        ...
        for k,v in data.iteritems():
            if k in OBSPROP:
                ...
                if DSID:
                    ...
                    sql = """select to_char("RESULT_TIME"::timestamptz at time zone 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS"Z"') as "LAST_DATE" from "OBSERVATIONS2" where "DATASTREAM_ID" = %s order by "RESULT_TIME"::timestamptz DESC limit 1""" % DSID[0][0]
                    connection.connect()
                    ...
                    if datetime.strptime(RESTIME, '%Y-%m-%dT%H:%M:%SZ') > datetime.strptime(LASTDATE[0][0], '%Y-%m-%dT%H:%M:%SZ'):  # 2018-09-01T21:00:00Z)
                        ...
                        print("%s - INFO: OBSERVATION FOR TIME %s ALREADY STORED IN DB FOR DS ID %s" % (datetime.now().isoformat(),datetime.strptime(RESTIME, '%Y-%m-%dT%H:%M:%SZ'), DSID[0][0]))
                else:
